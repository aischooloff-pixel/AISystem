...
class AIService:
    ...
    def finalize(self, data: dict) -> dict:
        ...
        if (
            data.get("status") in ("cold", "warm")
            and data.get("confidence", 100) < self.confidence_threshold
        ):
            ...
            # ответивший «да просто захотелось больше бабок», получал
            # «передал информацию Юлии» и разговор обрывался (прод 29.07).
            # Юлия видит таких в CRM; к ней они попадут, когда скажут
            # о готовности. Отступление от раздела 11 — по решению Ивана 29.07.
            logger.info(
                "Статус %s с уверенностью %s — разговор продолжается, передачи нет",
                data["status"],
                data["confidence"],
            )
